...
void main() {
  ...
  test('Export mystical background to PNG from Flutter painter 1:1', () async {
    const Size outputSize = Size(1440, 3200);
    final Rect fullRect = Offset.zero & outputSize;

    final ui.PictureRecorder recorder = ui.PictureRecorder();
    final Canvas canvas = Canvas(recorder);

    final Paint backgroundPaint = Paint()
      ..shader = AppColors.appBackgroundGradient().createShader(fullRect);
    canvas.drawRect(fullRect, backgroundPaint);

    final Paint overlayPaint = Paint()
      ..shader = AppColors.nebulaOverlayGradient().createShader(fullRect);
    canvas.drawRect(fullRect, overlayPaint);

    const _ExportSacredPatternPainter().paint(canvas, outputSize);
    ...
    final Directory outputDir = Directory('assets/images/generated');
    outputDir.createSync(recursive: true);
    final File outputFile = File('${outputDir.path}/mystical_background.png');
    outputFile.writeAsBytesSync(byteData!.buffer.asUint8List());
  });
}

class _ExportSacredPatternPainter extends CustomPainter {
  const _ExportSacredPatternPainter();
